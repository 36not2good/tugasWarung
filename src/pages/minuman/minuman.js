...
export default class Minuman extends Component {
  ...
  componentDidMount() {
    axios
      .get(API_URL + "produks")
      .then((res) => {
        const minumanMenus = res.data.filter(
          (menu) => menu.kategoris.nama === "minuman"
        );
        this.setState({ menus: minumanMenus });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  handleDrinkClick = (Drink) => {
    const { selectedDrinks, totalHarga } = this.state;
    ...
    if (existingDrink) {
      const updatedDrinks = selectedDrinks.map((selectedDrink) =>
        selectedDrink.id === Drink.id
          ? {
            ...selectedDrink,
            quantity: selectedDrink.quantity + 1,
            totalPrice: selectedDrink.harga * (selectedDrink.quantity + 1),
          }
          : selectedDrink
      );
      ...
      this.setState({
        ...
        totalHarga: totalHarga + Drink.harga,
      });
    } else {
    ...
    const updatedDrinks = selectedDrinks.map((selectedDrink) =>
      selectedDrink.id === editingDrink.id
        ? {
          ...
          note: editNote,
          totalPrice: editingDrink.harga * editQuantity,
        }
        : selectedDrink
    );
  ...
  handleDeleteMenu = () => {
    ...
    const updatedDrinks = selectedDrinks.filter(
      (selectedDrink) => selectedDrink.id !== editingDrink.id
    );
  ...
  handlePayOrder = () => {
    ...
    console.log("minuman yang dibeli:", selectedDrinks);
              ...
              <div className="Drink-wrap">
                {menus.map((Drink) => (
                  <div
                    ...
                    onClick={() => this.handleDrinkClick(Drink)}
                  >
                    <div className="Drink-image-container">
                      <FontAwesomeIcon
                        ...
                        className="icon-info"
                      />
                      <img src={`/image/minuman/${Drink.image}`} alt={Drink.nama} className='Drink-image' />
                    </div>
                    <div className="Drink-details">
                      <div className="Drink-name">
                        <h3>{Drink.nama}</h3>
                        <p>{Drink.warungs.nama}</p>
                      </div>
          ...
          <div className="payment-details">
            ...
            <div className="selected-Drinks">
              {selectedDrinks.map((selectedDrink) => (
                ...
                >
                  <p>
                    {selectedDrink.nama} ({selectedDrink.quantity}){" "}
                  </p>
